...
- [ ] 작업 1b: BusAlertService.kt 추가 축소 (1,620줄 → ~1,200줄 목표) — 세션 4개로
  ...
  - [x] 1b-3: 도착 확인/추적정보 갱신 클러스터를 신규 협력 클래스로 (가장 큰 감소,
        ...
        실기기 검증 대부분 완료(수동/전체 알람 시작·중지, 취소 연타, devlog
        2026-07-28 (5~6차) 참조) — 자동알람 시작/타임아웃/중복방지와 stationId
        보정만 아직 미완(`docs/topics/follow-up-status.md` 참조). "앱 스와이프
        종료" 경로는 실제로 존재하지 않는 코드 경로였음을 확인·정정함
