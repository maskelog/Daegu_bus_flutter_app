...
## 우선순위 높음: 1.0.4+69 Play 내부 테스트 업로드 확인

- 현재 상태: `1.0.4+68`은 Play 내부 테스트에 등록되어 있다. Flutter 3.47.2 전환과
  배터리 최적화 진입 차단 수정을 포함한 서명·난독화 AAB `1.0.4+69`는 로컬에서
  생성·검사했지만 Play Console에는 아직 업로드하지 않았다.
- 완료 기준: Play Console이 `versionCode 69`를 수락하고 내부 테스트에 배포한 뒤,
  **Play 배포본**을
