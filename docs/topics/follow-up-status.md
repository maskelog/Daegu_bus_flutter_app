...
## 우선순위 중간: BusAlertService.kt 1b 자동알람 실기기 검증

- 현재 상태: 1b-1~3(1,620→1,101줄) 병합 후 수동 알람 시작/중지, 다중 추적 →
  전체 중지, 취소 연타는 2026-07-28에 실기기(Galaxy Note10+)로 확인했다
  (devlog 2026-07-28 (5~6차) 참조). 자동알람 시작·타임아웃·중복방지·중지와
  stationId 보정 재시도는 아직 확인 못 했다 — 기존 자동알람이 트리거까지
  1시간 이상 남아있어 실제 대기 없이는 검증 불가했다.
- 완료 기준: 트리거 임박한 자동알람을 등록해 실제 발화·타임아웃·중지 3경로를
  실기기로 확인한다.
