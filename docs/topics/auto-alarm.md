...
> 마지막 갱신: 2026-07-28 (반복 회차 체인·중복 방지·stationId 보정 반영)
...
- 사전 추적이 발화하면 다음 반복 회차는 `targetAlarmTime` 이후를 기준으로 계산한다.
  현재 시각만 기준으로 계산하면 아직 미래인 오늘 본 시각을 같은 회차로 다시 선택해
  3~5초 간격 재발화 루프가 생기므로, `findNextTargetTimeAfterOccurrence`를 사용한다.
...
- 저장된 알람의 `isActive=false`는 모든 복원·새로고침 경로에서 예약 차단 조건이다.
  `AlarmService.loadAutoAlarms()`는 비활성 알람을 활성 목록이나 스케줄러에 넣지 않고,
  남아 있을 수 있는 현행·레거시 네이티브 예약과 재부팅 재등록 저장소 항목을 정리한다.
- 자동알람은 설정 시각 5분 전부터 추적할 수 있지만, ETA가 바뀌었다는 이유만으로 사전
  진동하지 않는다. `NotificationHandler`는 남은 정거장 1~2개 또는 3분 이내(`곧 도착`
  포함)일 때만 ETA 변경 진동을 허용한다. 수동 승차 알람의 ETA 변경 진동은 유지한다.
  `remainingStops="0"`은 정보 없음 placeholder이므로 임박으로 판정하지 않는다.
- 같은 노선의 자동알람이 이미 pending 또는 active이면 동기 진입 단계에서 중복을
  거부한다. 활성 상태인 같은 노선 요청은 경과 시간으로 새 회차를 추정하지 않는다.
  정상 다음 회차 전에는 기존 추적이 설정된 타임아웃으로 종료되어야 한다.
- 자동알람 진입 시 stationId가 `7`로 시작하는 10자리 숫자 형식이 아니면
  `wincId`/노선 정류장 목록/bsId fallback 순으로 보정한 뒤, 유효한 결과일 때만
  실시간 추적을 시작한다.
