 ...
 * 정확한 알람·배터리 최적화 상태·실시간(promoted) 알림 권한 조회 및 설정 화면 이동을 담당한다.
 */
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            "canScheduleExactAlarms" -> {
                // Android 12+에서는 사용자가 특수 앱 액세스에서 허용해야 한다.
            ...
            "requestIgnoreBatteryOptimizations" -> {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    try {
                        ...
                        if (resolveInfo == null) {
                            result.error("BATTERY_OPTIMIZATION_INTENT_NOT_FOUND", "설정 화면을 열 수 없습니다.", null)
                            return
                        }
