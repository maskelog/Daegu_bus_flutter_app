...
class BusAlertTrackingManager(
    ...
    private val arrivalMonitor = BusAlertArrivalMonitor(
        ...
        useTextToSpeechProvider = useTextToSpeechProvider,
        arrivalThresholdMinutes = arrivalThresholdMinutes,
        service = service,
        hasNotifiedArrival = hasNotifiedArrival,
        ongoingNotificationId = ongoingNotificationId,
        notificationHandler = notificationHandler,
        isManuallyStoppedByUserProvider = isManuallyStoppedByUserProvider,
        setManuallyStoppedByUser = setManuallyStoppedByUser,
        setLastManualStopTime = setLastManualStopTime,
        lastManualStopTimeProvider = lastManualStopTimeProvider,
        restartPreventionDurationMs = restartPreventionDurationMs,
        alertOnArrivalOnlyProvider = alertOnArrivalOnlyProvider,
        stopTrackingForRoute = { routeId, cancelNotification ->
    ...
    // 버스 정보 갱신·도착 판정 구현은 BusAlertArrivalMonitor가 담당한다.
    fun updateBusInfo(routeId: String, stationId: String, stationName: String) {
        arrivalMonitor.updateBusInfo(routeId, stationId, stationName)
    }
    ...
    fun updateBusInfoFromFlutter(
        ...
        arrivalMonitor.updateBusInfoFromFlutter(
            routeId,
            busNo,
            stationName,
            remainingMinutes,
    ...
    fun checkNextBusAndNotify(trackingInfo: TrackingInfo, newBusInfo: BusInfo) {
        arrivalMonitor.checkNextBusAndNotify(trackingInfo, newBusInfo)
    }

    fun checkArrivalAndNotify(trackingInfo: TrackingInfo, busInfo: BusInfo) {
        arrivalMonitor.checkArrivalAndNotify(trackingInfo, busInfo)
    }
    ...
    fun updateTrackingInfoFromFlutter(
        ...
        arrivalMonitor.updateTrackingInfoFromFlutter(
            routeId,
            busNo,
            stationName,
            remainingMinutes,
