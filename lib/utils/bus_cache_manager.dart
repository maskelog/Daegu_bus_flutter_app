...
class BusCacheManager {
  static const int CACHE_DURATION_SECONDS = 300; // 5분
  static const int MAX_CACHE_ENTRIES = 50; // 최대 캐시 항목 수
  static const String CACHE_PREFIX = 'bus_cache_';
  static const String TIMESTAMP_PREFIX = 'bus_timestamp_';
  static const String CACHE_KEYS = 'cache_keys';
  ...
  bool isValidCache(String key) {
    final timestampKey = '$TIMESTAMP_PREFIX$key';
    final cachedTime = _prefs?.getInt(timestampKey);
    
    if (cachedTime == null) return false;
    
    final cacheAge = DateTime.now().millisecondsSinceEpoch - cachedTime;
    return cacheAge < (CACHE_DURATION_SECONDS * 1000);
  }

  /// 버스 도착 정보 캐시 저장
  Future<void> cacheBusArrivals(String stationId, List<BusArrival> arrivals) async {
    await initialize();
    ...
    final key = '$CACHE_PREFIX$stationId';
    final timestampKey = '$TIMESTAMP_PREFIX$stationId';
  ...
  Future<void> cacheBusStops(String searchKey, List<BusStop> stops) async {
    ...
    final key = '${CACHE_PREFIX}stations_$searchKey';
    final timestampKey = '${TIMESTAMP_PREFIX}stations_$searchKey';
  ...
  Future<void> cleanExpiredCache() async {
    ...
    for (final key in expiredKeys) {
      await _prefs?.remove('$CACHE_PREFIX$key');
      await _prefs?.remove('$TIMESTAMP_PREFIX$key');
    }
    
    if (expiredKeys.isNotEmpty) {
      final remainingKeys = cacheKeys.where((k) => !expiredKeys.contains(k)).toList();
      await _prefs?.setStringList(CACHE_KEYS, remainingKeys);
    }
  }
  ...
  Future<void> _updateCacheKeys(String key) async {
    ...
    if (!keys.contains(key)) {
      keys.add(key);
      await _prefs?.setStringList(CACHE_KEYS, keys);
    }
  }
  ...
  Future<List<String>> _getCacheKeys() async {
    return _prefs?.getStringList(CACHE_KEYS) ?? [];
  }

  /// 캐시 크기 제한 적용
  Future<void> _enforeCacheLimit() async {
    final keys = await _getCacheKeys();
    ...
    if (keys.length > MAX_CACHE_ENTRIES) {
      ...
      for (final key in keys) {
        final timestamp = _prefs?.getInt('$TIMESTAMP_PREFIX$key') ?? 0;
      ...
      final keysToRemove = keysWithTimestamp
          .take(keysWithTimestamp.length - MAX_CACHE_ENTRIES)
          .map((e) => e.key)
          .toList();
      ...
      final remainingKeys = keys.where((k) => !keysToRemove.contains(k)).toList();
      await _prefs?.setStringList(CACHE_KEYS, remainingKeys);
    }
  }
}
